...
Google's Gemini line of models have been making waves lately, especially with the new 2.5 Pro model, boasting impressive benchmark scores that have the community buzzing.

I've been using Gemini models in production for a few months, and I'm calling the bell. There's something going on with these models that the benchmarks don't show.
...
- it's the impressive context window (2M tokens)
...
On paper, it seemed like our product perfectly matched with Gemini, and our evals corroborated. It really did perform well for document analysis.

In this example, Gemini was able to find and read data in a scanned report on page 77 of a 88 page document.
...
My approach to fix this was two pronged:
...
Nonetheless, I tried what I could by:

- Adding more descriptions to each tool definition
- Adding conversation examples with tool calls
- Emphasizing the the AI should call the tools after mentioning them
- Reducing the model temperature

At this point I was convinced it was code error. So we go to Fix #2.

# Is it code error?

I read up on Gemini docs and actioned the following:

- Switch to Google's newest genai SDK (watch for a separate blog article coming on confusions with all the different SDKs for Gemini)
- Use Google's more reliable AFC (Automatic Function Calling)
- Switch to Google's Enterprise API, Vertex AI

If it was logic error that we didn't call the tool, then these would surely solve it.
